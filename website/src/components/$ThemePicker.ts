...
import { $svg, attr, component, INode, nodeEvent, style, stylePseudo } from '@aelea/dom'
import { $icon } from "@aelea/ui-components"
import { setTheme } from "@aelea/ui-components-theme"
import { snapshot, startWith } from "@most/core"
import { pallete, theme, themeList } from "../theme"




export const $Picker = (themeNameList: string[]) => component((
  [changeThemeEffect, changeThemeEffectTether]: Behavior<INode, string>
) => {

  const currentTheme = startWith(theme.name, changeThemeEffect)
  ...
  return [
    O(
      ...
      changeThemeEffectTether(
        nodeEvent('click'),
        snapshot((current) => {
          const toIdx = (themeNameList.indexOf(current) + 1) % themeNameList.length
          const toTheme = themeNameList[toIdx]
