...
export function readDomTheme() {
  ...
  const configNode = document.querySelector<HTMLStyleElement>(
    `style#${configStyleId}`,
  )
  if (!configNode || !configNode.sheet) {
    throw new Error(
      `CRITICAL: <style id="${configStyleId}"> not found or missing sheet. Cannot discover themes.`,
    )
  }
  ...
  if (!configSheet) {
    throw new Error(
      `CRITICAL: Stylesheet with name="${configStyleId}" not found. Cannot discover themes.`,
    )
  }

  if (
    !configSheet.cssRules ||
    !(configSheet.cssRules instanceof CSSRuleList) ||
    configSheet.cssRules.length === 0
  ) {
    throw new Error(
  ...
  if (storedThemeName) {
    ...
    if (prefersDark) {
      ...
      if (darkTheme) {
        ...
        console.warn(
          'System prefered dark theme not found. Falling back to the first theme in the list.',
        )
      }
    }
  }
