import { empty, type ISink, type IStream, type ITime, isFunction, merge, never, propagateRunEventTask } from '@/stream'
import { stream } from '@/stream-extended'
import type { I$Node, I$Op, I$Text, INode, INodeCompose, INodeElement, ITextNode } from './types.js'
...
export function createNode<TElement>(
  createElement: () => INodeElement<TElement>,
  postOp: I$Op<TElement> = x => x
): INodeCompose<TElement> {
  const nodeComposeFn = (...input: (I$Op<unknown> | I$Node<unknown>)[]): INodeCompose | I$Node => {
    ...
    const $segments = input.length ? (input as I$Node<TElement>[]) : [never]
