...
export function eventElementTarget<
  A extends ElementEventNameList,
  ...
  return {
    run(sink, scheduler) {
      ...
      const removeListener = () =>
        element.removeEventListener(eventType, cb, options)
...
export const nodeEvent: NodeEvent = curry2((eventType, node) => {
  return join(
    map(
      (ns) => eventElementTarget(eventType, ns.element, { capture: true }),
      node,
    ),
  )
})
