...
import { combineMap, fromIterable, merge, op, tap, zipMap } from '../src/stream/index.js'
...
bench
  ...
  .add(`@aelea merge nested: ${n} streams of ${m} numbers`, () => {
    const streams = arrays.map(arr => fromIterable(arr))
    return runStream(
      op(
        merge(...streams),
        tap((x: number) => x)
  ...
  .add(`@aelea combine nested: 3 streams of ${m} numbers`, () => {
    const streams = arrays.slice(0, 3).map(arr => fromIterable(arr))
    return runStream(
      op(
