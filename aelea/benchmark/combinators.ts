...
bench
  ...
  .add(`@aelea merge 2 streams of ${n} numbers`, () => {
    const s1 = fromIterable(numbers)
    const s2 = fromIterable(numbers)
    return runStream(merge(s1, s2))
  })
  ...
  .add(`@aelea merge 5 streams of ${n} numbers`, () => {
    const streams = Array.from({ length: 5 }, () => fromIterable(numbers))
    return runStream(merge(...streams))
  })
  ...
  .add(`@aelea combine 2 streams of ${n} numbers`, () => {
    const s1 = fromIterable(numbers)
    const s2 = fromIterable(numbers)
    return runStream(combineMap(sum, s1, s2))
  })
  ...
  .add(`@aelea combine 3 streams of ${n} numbers`, () => {
    const s1 = fromIterable(numbers)
    const s2 = fromIterable(numbers)
    const s3 = fromIterable(numbers)
    return runStream(combineMap(add3, s1, s2, s3))
  })
  ...
  .add(`@aelea zip 2 streams of ${n} numbers`, () => {
    const s1 = fromIterable(numbers)
    const s2 = fromIterable(numbers)
    return runStream(zipMap(sum, s1, s2))
  })
  ...
  .add(`@aelea zip 3 streams of ${n} numbers`, () => {
    ...
    const s2 = fromIterable(numbers)
    const s3 = fromIterable(numbers)
    return runStream(zipMap(add3, s1, s2, s3))
  })
